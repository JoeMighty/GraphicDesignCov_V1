...
import { shuffleItems } from "./layout";
...
export default function GalleryGrid({
  ...
  // mount — so the order reshuffles on every visit/refresh without causing
  ...
  const ordered = useMemo(() => shuffleItems(items, sessionSeed), [items, sessionSeed]);
  ...
  return (
    <div className="columns-1 gap-x-6 px-6 pb-48 sm:columns-2 sm:gap-x-10 md:columns-3 lg:columns-4 xl:columns-5">
      {ordered.map((item, i) => (
        <motion.article
          key={item.id}
          ...
          layout
          className="relative z-20 mb-16 break-inside-avoid sm:mb-24"
        >
          <button
